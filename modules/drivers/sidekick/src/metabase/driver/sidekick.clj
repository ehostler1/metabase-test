(ns metabase.driver.sidekick
  ...
  (defmethod sql-jdbc.conn/connection-details->spec :sidekick
    [_ {:keys [local-user local-password admin-user admin-password db host port domain]
        :or   {local-user "customReports_{local}"}
        :as   details}]
    (let [local (get (get (deref (driver-api/current-user)) :login_attributes) :local)]
